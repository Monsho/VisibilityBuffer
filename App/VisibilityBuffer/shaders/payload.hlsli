...
struct [raypayload] MaterialPayload
{
	float3	emissive				: read(caller) : write(closesthit);			// float emissive.xyz
	float	pad						: read(caller) : write(closesthit);
	uint2	normalRoughnessMetallic	: read(caller) : write(closesthit);			// 16bit unorm normal.xyz + 8bit roughness + 8bit metallic = 32bit x 2
	uint	baseColorUnorm			: read(caller) : write(closesthit);			// 8bit unorm baseColor.xyz + 8bit flag
	float	hitT					: read(caller) : write(caller, closesthit, miss);	// ray hit T
};
